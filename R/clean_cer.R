...
imput_residential_kwh <- function(DT_KWH) {
...
# IMPUT WEEKDAYS ----------------------------------------------------------
  ...
  ## create weekday indx
  setkey(DT_wkdy, id, date_cer) # crucial that we sequentially
  ...
  DT_wkdy[is.na(kwh), kwh:=imputs]
  DT_wkdy[is.na(kw), kw:=kwh*2]

# IMPUT WEEKENDS ----------------------------------------------------------
  ## data table to search
  DT_wknd <- merge(DT1, weekends, by = 'date_cer') # weekend data
  DT_wknd[, hour_cer := date_cer %%100]
  DT_wknd[, day_cer  := (date_cer - hour_cer)/100]

  ## create weekday indx
  setkey(DT_wknd, id, date_cer) # crucial that we sequentially
  ids <- unique(DT_wknd$id)
  dates <- unique(DT_wknd$date_cer)
  days <- unique((dates - dates%%100)/100)
  N <- length(ids)
  T <- length(dates)
  NT <- matrix(DT_wknd$kwh,nrow=T,ncol=N) # rows = T, cols = N
  indx <- which(is.na(NT), arr.ind = TRUE) # find r,c index of missing vals
  indx <- as.data.table(indx)
  indx[, id := ids[col]]
  indx[, date_cer := dates[row]] # only works if data is sequential!
  ...
  indx[, day_cer  := (date_cer - hour_cer)/100]
  # set keys for quick search and column binding
  setkey(DT_wknd, id, hour_cer, day_cer)
  setkey(indx, id, hour_cer, day_cer)
  # parallel search
  start <- proc.time()
  if(require(parallel) & .Platform$OS.type == "unix") {
    ...
    imputs <- mcmapply(function(x, y, z) { # use mcmapply
      j <- which(days %in% z) # match to cer_days vector
      past_days <- days[(j-1):(j-10)] # sequence back 5 weekdays
      past_kwh <- vapply(past_days, function(i) DT_wknd[.(x,y,i)]$kwh, numeric(1))
      avg10 <- mean(past_kwh, na.rm=TRUE) # avg of past 10 readings at given hour
      return(avg10)
    }, x=indx$id, y=indx$hour_cer, z=indx$day_cer, mc.cores=CORES)
  } else {
  ...
  DT_wknd[is.na(kwh), kwh:=imputs]
  DT_wknd[is.na(kw), kw:=kwh*2]

# END IMPUT ------------------------------------------------------------
  DT1 <- rbindlist(list(DT_wknd, DT_wkdy))
  setkey(DT1, id, date_cer)
  return(DT1)
}
